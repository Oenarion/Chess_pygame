...
def create_pieces(spritesheet):            
    black_pieces = {}
    b_pawns = [p.Pawn(spritesheet, scale=2, is_white=0) for col in range(8)]
    b_rooks = [p.Rook(spritesheet, scale=2, is_white=0) for col in range(2)]
    b_knights = [p.Knight(spritesheet, scale=2, is_white=0) for col in range(2)]
    b_bishops = [p.Bishop(spritesheet, scale=2, is_white=0) for col in range(2)]
    ...
    white_pieces = {}
    w_rooks = [p.Rook(spritesheet, scale=2, is_white=1) for col in range(2)]
    w_pawns = [p.Pawn(spritesheet, scale=2, is_white=1) for col in range(8)]
    w_knights = [p.Knight(spritesheet, scale=2, is_white=1) for col in range(2)]
    w_bishops = [p.Bishop(spritesheet, scale=2, is_white=1) for col in range(2)]
...
def main():
    ...
    game_grid = grid.Grid(8, 8, TILE_SIZE, BORDER)
    game_grid.populate_grid(black_pieces, white_pieces, False)
    grid_colors = [WHITE, GREEN]
    running = True
    while running:
        screen.fill((0, 0, 0))
        game_grid.draw(screen, grid_colors)
